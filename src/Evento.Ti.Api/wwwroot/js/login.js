...
async function handleLoginSubmit(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Sprint 1 — Controle básico de sessão: salva token se existir
            const token =
                data?.token ||
                data?.accessToken ||
                data?.jwt ||
                data?.data?.token; // fallback defensivo
            ...
            // Sprint 1 — Redirecionamento pós-login
            // Observação: usando /index.html (já existe no projeto). Depois podemos apontar para /dashboard.html.
